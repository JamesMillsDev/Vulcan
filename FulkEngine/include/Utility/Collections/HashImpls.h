...
{
	template<>
	struct hash<Fulk::Color>
	{
		uint64 operator()(const Fulk::Color& color) const noexcept
		{
			return Fulk::HashAll(color.r, color.g, color.b, color.a);
		}
	};
	...
	struct hash<vec2>
	{
		...
		{
			return Fulk::HashAll(vec.x, vec.y);
		}
	};
	...
	struct hash<vec3>
	{
		...
		{
			return Fulk::HashAll(vec.x, vec.y, vec.z);
		}
	};
	...
	struct hash<vec4>
	{
		...
		{
			return Fulk::HashAll(vec.x, vec.y, vec.x, vec.w);
		}
	};
	...
	struct hash<quat>
	{
		...
		{
			return Fulk::HashAll(quaternion.x, quaternion.y, quaternion.x, quaternion.w);
		}
	};
	...
	struct hash<mat4>
	{
		...
		{
			return Fulk::HashAll(matrix[0], matrix[1], matrix[2], matrix[4]);
		}
	};
}